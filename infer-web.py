    ...
    if architecture == "VR":
        try:
            ...
            if inp_root != "":
                ...
                if audio_wav_input_path != "" and audio_wav_input_path != None:
                    paths = [audio_wav_input_path]
                else:
            ...
            for path in paths:
                ...
                try:
                    ...
                    if (
                        ...
                        need_reformat = 0
                        print("Running uvr for path: ", inp_path)
                        predict_res = pre_fun._path_audio_(
        ...
        final_vocal_path, final_instrument_path = predict_res
        json_str = json.dumps({"vocal": final_vocal_path, "instrument": final_instrument_path})
        print("vocal and instrument path:", json_str)
        return [json_str]
...
def preprocess_dataset(trainset_dir, exp_dir, sr, n_p, dataset_path):
    ...
    with open(log_file_path, "w") as log_file:
        ...
        print("Calling preprocess.py with command ", cmd)
        p = subprocess.Popen(
...
def GradioRun(app):
    ...
    if config.iscolab or config.paperspace:
        app.queue(concurrency_count=20, max_size=max_size).launch(
            server_name="0.0.0.0",
