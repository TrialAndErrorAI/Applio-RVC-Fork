...
class VC:
    ...
    def vc_single(
        ...
        if input_audio_path1.startswith('http://') or input_audio_path1.startswith('https://'):
            print("Provided audio is a URL")
        elif input_audio_path1.startswith('data:'):
            print("Provided audio is a base64 string")
        elif (not os.path.exists(input_audio_path1)) and (not os.path.exists(os.path.join(now_dir, input_audio_path1))):
            return "Audio was not properly selected or doesn't exist", None

        resLoadAudio = load_audio(file=input_audio_path1,
                               sr=16000,
        ...
        if isinstance(resLoadAudio, tuple):
            input_audio_path1 = resLoadAudio[1]
            audio = resLoadAudio[0]
        else:
            audio = resLoadAudio
